...
async fn deploy_autoscaler(cli: &Cli) -> Result<()> {
    ...
    ResourceManager::apply_manifest(
        &kubeconfig_path,
        std::path::Path::new("manifests/autoscaler/01-namespace.yaml"),
    )
    .await?;
    ...
    let hcloud_token = config.get_hcloud_token()?;
    create_hcloud_secret(&kubeconfig_path, &hcloud_token).await?;
    ...
    create_talos_configmap(&kubeconfig_path, &worker_config_path).await?;

    // 4. Apply ServiceAccount and RBAC
    info!("Creating ServiceAccount and RBAC...");
    ResourceManager::apply_manifest(
        &kubeconfig_path,
        std::path::Path::new("manifests/autoscaler/02-serviceaccount.yaml"),
    )
    .await?;
    ResourceManager::apply_manifest(
        &kubeconfig_path,
        std::path::Path::new("manifests/autoscaler/03-rbac.yaml"),
    )
    .await?;

    // 5. Generate and apply Deployment with dynamic configuration
    info!("Deploying autoscaler...");
    create_autoscaler_deployment(&kubeconfig_path, &config, autoscaler_config).await?;
...
async fn create_hcloud_secret(kubeconfig_path: &std::path::Path, token: &str) -> Result<()> {
    use crate::utils::command::CommandBuilder;
...
async fn create_talos_configmap(
...
async fn create_autoscaler_deployment(
    kubeconfig_path: &std::path::Path,
    config: &ClusterConfig,
